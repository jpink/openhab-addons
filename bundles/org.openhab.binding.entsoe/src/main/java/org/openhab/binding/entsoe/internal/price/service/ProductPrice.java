...
public record ProductPrice(double price, @Nullable VatRate vatRate, double vatAmount, double total,
                           CurrencyUnit2 currency, Unit<Energy> unit) {
    public static ProductPrice fromPrice(double price, VatRate vatRate, CurrencyUnit2 currency, Unit<Energy> unit) {
        return new ProductPrice(price, vatRate, vatRate.vatFromPrice(price), vatRate.totalFromPrice(price), currency,
                unit);
    }

    public static ProductPrice fromTotal(double total, VatRate vatRate, CurrencyUnit2 currency, Unit<Energy> unit) {
