...
public class PriceHandler extends BaseThingHandler {
    ...
    public PriceHandler(Thing thing, EntsoeHandlerFactory handlerFactory) {
        ...
        var id = thing.getUID();
        current = new ChannelUID(id, "current");
    }
    ...
    private void handleUpdateCurrentPrice() {
        try {
            ...
            if (price == null) {
                updateStatus(ThingStatus.UNKNOWN);
                //TODO updateState(current, null);
            } else {
                var total = price.total();
                var amount = total.total();
                var quantity = Quantities.getQuantity(new BigDecimal(amount), total.unit(),
                        Quantity.Scale.RELATIVE); // Prices are relative scale (there is no natural minimum)
                var q = new QuantityType<Energy>();
                updateState(current, new QuantityType<>(amount, total.unit())); // This is the best what is possible.
                updateState(current, new DecimalType(amount));
                //updateState(current, PercentType.valueOf(""));
            }
